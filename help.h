...
bool isKeyPressed(int& whichKey)   //whichKey passed as reference.... 
{
	...
	key = GetAsyncKeyState(0x50);// 'P' key for pause
		if (key == 1)
	{

		whichKey = 8;   // 8 if pause key is pressed
		return true;
	}
...
void setTextColor(int R, int G, int B) {
	...
	int color = (R > 127) << 4 | (G > 127) << 2 | (B > 127); // convert RGB to console color
	SetConsoleTextAttribute(hConsole, color);
}
// Function to print text at specified position
void myText(int x, int y, const string& text ,int R = 255, int G = 255, int B = 255) {
	setTextColor(R, G, B); // set the color
	gotoxy(x / 8, y / 16); // adjust scaling for console rows and columns
	cout << text;
	setTextColor(255, 255, 255); // reset to white after printing
}